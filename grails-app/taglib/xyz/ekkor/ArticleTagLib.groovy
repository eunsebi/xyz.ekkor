...
<i class="${category.iconCssNames}"></i> ${g:message([code: category.labelCode, default: category.defaultLabel])}</a>""")
    ...
    def tags = { attrs, body ->
        ...
        for(int i = 0; i < limit; i++) {
            def tag = tags[i]
            out << raw("""<a href='${request.contextPath}/articles/tagged/${tag}' class='list-group-item-text item-tag label ${classNames}'>${tag}</a> """)
        }
    }
    ...
    def voteButtons = { attrs, body ->
        ...
        if(category.useEvaluate) {
            ...
            out << raw("""<div class="note-evaluate-wrapper">""")
            ...
            if(!vote || vote?.point > 0) {
                out << raw("""<a href="javascript://" class="note-vote-btn" role="button" data-type="${assentVotedClass}" data-eval="${category.useEvaluate}" data-id="${content.id}">""")
                out << raw("""<i id="note-evaluate-assent-${content.id}" class="fa fa-angle-up note-evaluate-assent-${assentVotedClass}" data-placement="left" data-toggle="tooltip" title="${assentVotedClass == 'unvote' ? '추천 취소' : '추천'}"></i></a>""")
            } else {
                out << raw("""<a href="javascript://" class="note-vote-btn" role="button" data-type="disabled" data-eval="${category.useEvaluate}" data-id="${content.id}">""")
                out << raw("""<i id="note-evaluate-assent-${content.id}" class="fa fa-angle-up note-evaluate-assent-disabled"></i></a>""")
            }

            out << raw("""<div id="content-vote-count-${content.id}" class="content-eval-count">${shortenNumber(content.voteCount)}</div>""")

            if(!vote || vote?.point < 0) {
                out << raw("""<a href="javascript://" class="note-vote-btn" role="button" data-type="${dissentVotedClass}" data-eval="${category.useEvaluate}" data-id="${content.id}">""")
                out << raw("""<i id="note-evaluate-dissent-${content.id}" class="fa fa-angle-down note-evaluate-dissent-${dissentVotedClass}" data-placement="left" data-toggle="tooltip" title="${dissentVotedClass == 'unvote' ? '반대 취소' : '반대'}"></i></a>""")
            } else {
                out << raw("""<a href="javascript://" class="note-vote-btn" role="button" data-type="disabled" data-eval="${category.useEvaluate}" data-id="${content.id}">""")
                out << raw("""<i id="note-evaluate-dissent-${content.id}" class="fa fa-angle-down note-evaluate-dissent-disabled"></i></a>""")
            ...
            out << raw("""<a href="javascript://" class="note-vote-btn" role="button" data-type="${votedClass}" data-eval="${category.useEvaluate}" data-id="${content.id}">""")
            out << raw("""<i  id="note-vote-${content.id}" class="fa fa-thumbs-up note-${votedClass}" data-toggle="tooltip" data-placement="left" title="${vote ? '추천 취소' : '좋아요'}"></i></a>""")
            out << raw("""<div id="content-vote-count-${content.id}" class="content-count">${shortenNumber(content.voteCount)}</div>""")
        }
    }
